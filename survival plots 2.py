...
# ══════════════════════════════════════════════════════
# Train RF on MSK → Predict on METABRIC
# ══════════════════════════════════════════════════════
RF_PARAMS = dict(
    n_estimators    = 100,
    max_depth       = 4,
...
rf.fit(msk_X.values, msk_y)
print(f"\nRF trained on MSK ({len(msk_X)} patients) ✓")

meta_proba = rf.predict_proba(meta_X.values)[:, 1]

print(f"\nMETABRIC predictions:")
...
p_dfs = plot_km(
    time_col  = DFS_TIME,
    event_col = DFS_EVENT,
    title     = 'Disease Free Survival — METABRIC\n(Train MSK → Test METABRIC, excl. Age & Meno.)',
    save_path = os.path.join(SAVE_DIR, 'km_dfs_crosscohort.png'),
    df        = surv_dfs
)
hr_dfs, ci_l_dfs, ci_h_dfs, p_cox_dfs = run_cox(
    DFS_TIME, DFS_EVENT, 'DFS — Cross-cohort', surv_dfs
)
...
p_os = plot_km(
    ...
    save_path = os.path.join(SAVE_DIR, 'km_os_crosscohort.png'),
    df        = surv_os
)
hr_os, ci_l_os, ci_h_os, p_cox_os = run_cox(
    OS_TIME, OS_EVENT, 'OS — Cross-cohort', surv_os
)
...
print(f"{'='*65}")
print(f"  Model    : RF trained on MSK ({len(msk_X)} patients)")
print(f"  Test     : METABRIC ({len(meta_X)} patients)")
print(f"  Features : Excl. Age & Menopausal Status")
